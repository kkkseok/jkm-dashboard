...
export const SALES_MAPPING = {
  ...
  fields: {
    ...
    M: 'M', // 원가
    Q: 'Q', // 물류비
    R: 'R', // 이익액(공급가). 2026-06-18 sales_status_mod_* 파일부터 물류비(Q) 차감 반영(product BB 와 정렬). 수식은 R 을 그대로 사용.
    S: 'S', // 이익률(공급가)
    T: 'T', // 이익액(판매가)
    U: 'U', // 이익률(판매가)
  },
...
export const PRODUCT_MAPPING = {
  ...
  fields: {
    ...
    // 최종이익률은 기본적으로 파이프라인에서 **재계산**한다: 총최종이익액 / 공급가(L).
    //   (구성별 BC=BB/공급액(AV)×100, Σ공급액(AV)=L → 단품은 BC/100 과 동일, 묶음은 ΣBB/L.)
    //   단 공급가(L)=0/null 이면 재계산이 불가(0으로 나눔)하므로 이 BC(대표행)를 폴백으로 읽어
    //   파일값을 그대로 표시한다(2026-06-24 사용자 확정, 64df308 이전 동작 복원).
    //   BC 는 퍼센트 수치라 파이프라인에서 /100 해 비율(0~1)로 변환. 공급액 0 단품은 원본이 -100 센티넬.
    finalProfitRate: 'BC', // 최종이익률(%) — 공급가 0 폴백 전용 (정상은 ΣBB/L 재계산)
  },
} as const
