...
### 3-1. 원가 / 최종이익액 / 최종이익률 (계산 아님, 파일 값) — v1.9 (2026-06-12 사용자 확정)

실제 파일(`revenue_profit_product_0612.xlsx`)로 검증한 컬럼. 헤더가 2행 병합이라 눈으로 세면 어긋난다.

| 컬럼 | 출처(letter / 헤더) | 셀 서식 | 비고 |
|------|------|------|------|
| 원가 (`cost`) | **AZ** "원가" | `#,##0` | 표시 위치=공급가(L)와 이익액(R) 사이. product 매칭 실패 시 null |
| 최종이익액 (`finalProfit`) | **BB** "공급가기준 이익액" | `#,##0` | product 매칭 실패 시 null |
| 최종이익률 (`finalProfitRate`) | **BC** "공급가기준 이익율" | `#,##0.00"%"` | ⚠ raw 가 이미 퍼센트 수치(예: 17.52=17.52%) → 파이프라인에서 **/100** 해 비율(0~1)로 변환. 매칭 실패 시 null |

- ⚠ BC 서식은 Excel 진짜 %서식(내부값=비율)이 **아니라** `#,##0.00"%"`(숫자 뒤 문자 "%"). raw 17.52 는 0.1752 가 아니므로 /100 필수.
- 묶음(복합) 상품처럼 한 주문번호에 product 행이 여러 개면 **대표(첫) 행** 값을 그대로 쓴다(합산 안 함).
- letter 정의는 `src/lib/minus/mapping.ts` `PRODUCT_MAPPING.fields.cost/finalProfit/finalProfitRate`.
- cal_amount 셀 편집 재계산은 이 세 값을 건드리지 않는다(파일 값 보존).
...
| product 조인 실패 | **원가/최종이익액/최종이익률 = null** (AZ/BB/BC 를 읽을 행이 없음). UI에 "-" 표시 |
...
- 최종이익액/최종이익률은 **계산하지 않고** `revenue_profit_product` BB/BC 값을 그대로 표시(BC 는 /100 비율 변환). 원가는 AZ (v1.9 사용자 확정). ~~v1.2: R−Q~~ 폐기
