...
|------|------|------|------|
| 원가 (`cost`) | **BA** "원가총액" | `#,##0` | 표시 위치=공급가(L)와 이익액(R) 사이. 반품은 음수. product 매칭 실패 시 null. (AZ "원가"=단가는 미사용) |
...
- 한 **전표번호**에 product 행이 여러 개(진짜 묶음)면 **대표(첫=-001) 행** 값을 그대로 쓴다(합산 안 함). 한 주문에 전표가 여러 개면 각 라인이 별도 행·별도 대표값.
...
- 최종이익액/최종이익률은 **계산하지 않고** `revenue_profit_product` BB/BC 값을 그대로 표시(BC 는 /100 비율 변환). 원가는 BA(원가총액) (v1.9 사용자 확정). ~~v1.2: R−Q~~ 폐기
