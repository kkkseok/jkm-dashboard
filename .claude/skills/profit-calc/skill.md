...
| 최종이익률 (`finalProfitRate`) | **재계산** = 총최종이익액 / 공급가(L) | 파일 BC 를 읽지 않는다(매핑 없음). 구성별 BC=BB/공급액(AV)×100, Σ공급액(AV)=L → **단품은 BC/100 과 동일**(회귀 없음), 묶음은 ΣBB/L. L=0/null 또는 이익액 null → null |
...
- letter 정의는 `src/lib/minus/mapping.ts` `PRODUCT_MAPPING.fields.cost/finalProfit`(finalProfitRate 는 재계산이라 letter 없음).
