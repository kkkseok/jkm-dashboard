...
### DB / 서버 액션
- `src/db/schema/` — `cal_amount`(append-only, productCode UNIQUE 없음 → productCode 별 최신 1건이 winner), `product_master` + `product_channels`, `group_mapping`(group 모듈 전용 3테이블: `group_market_map`/`group_bundle_item`/`group_erp_code` — 기존 기능과 **완전 분리**, 업로드마다 통째 교체).
- `src/db/client.ts` — HMR 시 connection 누적 방지 싱글톤.
- Server Action 은 `src/lib/{cal-amount,products,group,group/gen}/actions.ts`.
- 마이그레이션 SQL 은 `drizzle/*.sql`. 스키마 수정 후 `pnpm db:generate` → `pnpm db:migrate`.
...
### 그룹 업로드 생성 (group 모듈, `src/lib/group/`)
사방넷 "미매핑 주문"(`no_mapping.xlsx`)을 사방넷 그룹상품 등록 파일(`group_upload.xlsx`, 13컬럼 A~M)로 변환한다. minus 와 마찬가지로 **엑셀은 클라이언트에서 파싱**하고 DB 조회만 Server Action 으로 한다. **2단계 / 2페이지** 구조:
...
- **묶음(복합) 역참조 키 = 사방넷코드(D), ★자체코드 아님.** ★A_B_… 코드는 구성품만 인코딩하고 **수량은 인코딩하지 않아**, 같은 구성·다른 수량 SKU(x3/x6/x12)가 ★코드를 공유 → ★키로 묶으면 첫 변형으로 수량이 뭉개진다(2026-06-17 버그, `drizzle/0007_fix_bundle_sabangnet_key.sql`). 내품 구성은 마스터 BG 수식 `(BG{행}*{수량})` 에서 추출.
- 그룹상품명(B) 생성 규칙은 `gen/name.ts` `buildGroupName`(단품/묶음 포맷, 묶음 2번째+ 내품의 브랜드 접두 제거). 채널 라벨은 minus 의 `normalizeSalesType` 를 **재사용**한다.

### 페이지 / 레이아웃
`src/app/(dashboard)/` route group + 공용 `layout.tsx`(사이드바·헤더). 구현된 페이지: `/minus`(분석), `/group-upload`(그룹 생성)·`/group-sources`(그룹 매핑 소스), `/products`(상품 마스터), `/cal-amount`(후정산금 관리). 테이블은 TanStack Table, 컬럼 표시/숨김·헤더 엑셀식 필터(매출구분/브랜드명/구분)·CSV 내보내기 포함. 사이드바엔 `/soldout`(품절 관리)만 **"예정"(disabled)** 으로 남아 있다 — `src/components/sidebar.tsx` `NAV_SECTIONS` 참조.
